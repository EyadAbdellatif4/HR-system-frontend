...
import { Package, ChevronLeft, ChevronRight, Plus, Trash2, Link2, Pencil } from 'lucide-react';
...
export function AssetsPage() {
  ...
  const handleDelete = async (asset: Asset) => {
    if (!window.confirm(`Are you sure you want to delete ${asset.label || asset.model || 'this asset'}?`)) {
      return;
    }
    ...
    try {
      ...
      const errorMessage = error?.data?.message || error?.message || 'Failed to delete asset. Please try again.';
      const formattedError = Array.isArray(errorMessage) ? errorMessage.join(', ') : errorMessage;

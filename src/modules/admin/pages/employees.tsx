...
import { LoadingSpinner, UserDetailModal, CreateUserModal, PaginationDropdown } from '@/shared/components';
...
import type { User as UserType, UserFilters, CreateUserRequest, UpdateUserRequest } from '@/types/api.types';
...
export function Employees() {
  ...
  // RTK Query hooks
  const { data: usersData, isLoading: loading, refetch } = useGetUsersQuery(filters);
  const [createUser] = useCreateUserMutation();
  const [updateUser] = useUpdateUserMutation();
  const [deleteUser] = useDeleteUserMutation();

  // Extract data
  const users = usersData?.users || usersData?.data?.items || [];
  const pagination = {
    total: usersData?.total || usersData?.data?.meta?.total || 0,
    page: usersData?.page || usersData?.data?.meta?.page || filters.page || 1,
    limit: usersData?.limit || usersData?.data?.meta?.limit || filters.limit || 10,
    totalPages: usersData?.totalPages || usersData?.data?.meta?.totalPages || 0,
  };
  ...
  const handleRowClick = async (user: UserType) => {
    // Fetch full user details
    try {
      const response = await dispatch(
        usersApi.endpoints.getUserById.initiate(user.id)
      ).unwrap();
  ...
  const handleUpdate = async (userId: string, updateData: UpdateUserRequest, files: File[] | null = null) => {
    try {
      // If files are provided, use the service directly with FormData
      if (files && files.length > 0) {
        const { userService } = await import('@/modules/admin/services');
        await userService.updateUser(userId, updateData, files);
      } else {
        // Use RTK Query for regular updates
  ...
  const handleCreate = async (data: CreateUserRequest, files: File[] | null = null) => {
    try {
      // If files are provided, use the service directly with FormData
      if (files && files.length > 0) {
        const { userService } = await import('@/modules/admin/services');
        ...
        // Use RTK Query for regular creation
  ...
  const getUserImageUrl = (user: UserType) => {
